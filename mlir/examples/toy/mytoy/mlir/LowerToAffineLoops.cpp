...
static void lowerOpToLoops(Operation *op, PatternRewriter &rewriter,
  ...
  // Insert an alloc and dealloc for the result of this operation.
  auto memRefType = convertTensorToMemRef(tensorType);
  auto alloc = insertAllocAndDealloc(memRefType, loc, rewriter);

  // Create a nest of affine loops, with one loop per dimenstion of the shape.
...
struct ConstantOpLowering : public OpConversionPattern<toy::ConstantOp> {
  ...
  matchAndRewrite(toy::ConstantOp op, OpAdaptor adaptor,
    ...
    // Create these constant up-front to avoid large amounts of redundant
    // operations.
    auto valueShape = memRefType.getShape();
    SmallVector<Value, 8> constantIndices;

    // So for a 2x3 tensor, it generates indices for the largest dimension: 0..2,
    // since the indices can also be used for the smaller dimension.
    if (!valueShape.empty()) {
    ...
    std::function<void(uint64_t)> storeElements = [&](uint64_t dimension) {
      ...
      // The arith.constant is also created here.
      ...
      // the list.
      // For dims d1..dn, it outer loops over d1 and inner loops sequentially
      // to where dn is the innermost loop.
  ...
  matchAndRewrite(toy::FuncOp op, OpAdaptor adaptor,
    ...
    if (op.getNumArguments() || op.getFunctionType().getNumResults()) {
      return rewriter.notifyMatchFailure(op, [](Diagnostic &diag) {
        diag << "expected 'main' to have 0 inputs and 0 result";
      });
    }
...
struct PrintOpLowering : public OpConversionPattern<toy::PrintOp> {
  ...
  matchAndRewrite(toy::PrintOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    // We don't lower toy.print in this pass, but we need to update it's
    // operands.
...
struct ReturnOpLowering : public OpConversionPattern<toy::ReturnOp> {
  ...
  matchAndRewrite(toy::ReturnOp op, OpAdaptor adaptor,
    ...
    // We lower toy.return to func.return
