...
public:
  ...
  matchAndRewrite(toy::PrintOp op, OpAdaptor adaptor,
    ...
    for (unsigned i = 0, e = memRefShape.size(); i != e; ++i) {
      ...
      // scf.yield is automatically inserted in the scr for loop body, but we want to
      // create a custom body, so we remove whatever is in the body of the loop.
      // scf.yield must always terminate a loop, so it is inserted after we add newline printfs.
      rewriter.setInsertionPointToEnd(loop.getBody());

      // Insert a newline after each of the inner dimensions of the shape.
      if (i < e - 1)
        LLVM::CallOp::create(rewriter, loc, getPrintfType(context), printRef,
                             newLineCst);

      // Now terminate the loop with the required scf.yield.
...
private:
  ...
  static LLVM::LLVMFunctionType getPrintfType(MLIRContext *context) {
    ...
    auto llvmFnTy = LLVM::LLVMFunctionType::get(llvmI32Ty, llvmPtrTy, /*isVarArg=*/true);
    return llvmFnTy;
  }
  ...
  static FlatSymbolRefAttr getOrInsertPrintf(PatternRewriter &rewriter,
    ...
    // InsertionGuard sets the PatternWriter's insertion location
    // and restores it when it's object goes out of scope.
  ...
  /// name, createing the string if necessary.
  static Value getOrCreateGlobalString(Location loc, OpBuilder &builder,
    ...
    // Pointer to the global string, which is an array of i8 (chars).
    Value globalPtr = LLVM::AddressOfOp::create(builder, loc, global);
    // Constant index with value 0.
    Value cst0 = LLVM::ConstantOp::create(builder, loc, builder.getI64Type(),
                                          builder.getIndexAttr(0));
    // Pointer to the first element of the global string.  The first index dereferences the pointer as
    // an Array of i8.  The second index indexes into the first char of the array.
...
void ToyToLLVMLoweringPass::runOnOperation() {
  ...
  // already exists a set of patterns to transform `affine` and `std`
